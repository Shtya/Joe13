...
export default function Input({ place , dataAos , error , cnInput , cn , label , type , icon , KEY , register , cnLabel}) {
  const t = useTranslations()
	...
	<div data-aos={dataAos} className={`${cn}  flex flex-col  gap-[5px] relative `} >
		...
		<div className={`border-b-[#BCBBBF]  w-full  overflow-hidden  border-b-[1px] relative  h-[45px]  ${cnInput} `} >
			<input className={` placeholder:capitalize focus:border-primary1  text16 w-full ${icon ? "rtl:pr-[40px] ltr:pl-[40px]" : "px-[10px]" } bg-transparent  h-full outline-none  `} {...register} id={KEY} placeholder={place} type={type} />
