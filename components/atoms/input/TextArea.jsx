...
export default function TextArea({ unite , place , dataAos , rounded=true , error , cnInput , classname , label , type , icon , KEY , register , cnLabel}) {
  const t = useTranslations()
	...
	<div data-aos={dataAos} className={`${classname} flex flex-col  gap-[5px]  relative `} >
		...
		<div className={`border-b-[#BCBBBF]  w-full   overflow-hidden  border-b-[1px] relative  h-[80px]  ${cnInput} `} >
			<textarea className={` p-[10px]  placeholder:text-[#9ca3af] text16 w-full h-full outline-none  bg-transparent `} {...register} id={KEY} placeholder={place} />
		</div>
